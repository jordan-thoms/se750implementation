...
public class ObjectBundle {
	...
	/**
	 * @deprecated Use {@link #commitToSerializer(boolean)} instead
	 */
	public boolean commitToSerializer()
	{
		return commitToSerializer(false);
	}

	public boolean commitToSerializer(boolean requiresPDstoreTripleFormat)
	{
		//Only do this if Serializer is set
		if(serializer!=null) {
			serializer.gatherData(this, requiresPDstoreTripleFormat);
			size=0;
			objects = new Vector<BSBMResource>(maxSize);
			return true;
		}else
